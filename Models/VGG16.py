...
save_path="C:\\Users\\jcac\\OneDrive - KTH\\Python\\CNN\\Journal-2\\Saved_models\\best_VGG16v3.pth"
save_probs_path="C:\\Users\\jcac\\OneDrive - KTH\\Python\\CNN\\Journal-2\\Results\\00_Probabilities\\VGG16\\"
...
def load_VGG16(num_classes, device):
...
def save_validation_csv(save_path, epoch, filenames_list, val_labels_list, predictions, all_probs):
    ...
    csv_file_path = os.path.join(save_path, f"VGG16_best_val_results.csv")
...
def evaluate_final_model(model, test_loader, criterion, device):
    ...
    plot_confusion_matrix(labels_list, predictions, class_names,"VGG16")
    ...
    csv_file_path = os.path.join(save_probs_path, "VGG16_test_probs.csv")
    df_probs.to_csv(csv_file_path, index=False)
...
transform = transforms.Compose([
    ...
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]) # Values from the ImageNet dataset where VGG16 was originally trained
